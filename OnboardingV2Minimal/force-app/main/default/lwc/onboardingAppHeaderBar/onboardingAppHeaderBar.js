...
import activateProgram from '@salesforce/apex/OnboardingAppActivationController.activate';
...
export default class VersionedRecordHeader extends LightningElement {
    ...
    get showActivateButton() {
        return this.normalizedStatus === 'draft' || this.normalizedStatus === 'inactive';
    }
    ...
    async handleActivate() {
        try {
            await activateProgram({ recordId: this.recordId, objectApiName: this.objectApiName });
            this.isActive = true;
            this.showToast('Activated', `${this.objectLabel} activated.`, 'success');
        } catch (error) {
            this.showToast('Activation Failed', error?.body?.message || error?.message || 'Unknown error', 'error');
        }
    }
    ...
    handleDeactivate() {
        ...
        updateRecord({ fields })
            .then(() => this.showToast('Success', `${this.objectLabel} deactivated.`, 'success'))
            .catch(error => this.showToast('Error', error?.body?.message || error?.message || 'Unknown error', 'error'));
    }
    ...
    handleKeyDown(event) {
        const items = Array.from(this.template.querySelectorAll('.custom-dropdown a'));
