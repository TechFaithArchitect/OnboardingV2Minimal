...
import getRequiredCredentials from '@salesforce/apex/OnboardingAppVendorProgramReqCtlr.getRequiredCredentials';
import createRequiredCredential from '@salesforce/apex/OnboardingAppVendorProgramReqCtlr.createRequiredCredential';
import getCredentialTypes from '@salesforce/apex/OnboardingAppVendorProgramReqCtlr.getCredentialTypes';
import createCredentialType from '@salesforce/apex/OnboardingAppVendorProgramReqCtlr.createCredentialType';
import updateRequiredCredentials from '@salesforce/apex/OnboardingAppVendorProgramReqCtlr.updateRequiredCredentials';
...
export default class VendorProgramOnboardingRequiredCredentials extends OnboardingStepBase {
    ...
    columns = [
        ...
        { label: 'Is Required', fieldName: 'Is_Required__c', type: 'boolean', editable: true },
        { label: 'Sequence', fieldName: 'Sequence__c', type: 'number', editable: true }
    ];
    ...
    handleFooterNextClick() {
        ...
        if (this.canProceed) {
            ...
            if (this.credentialsNeeded === null) {
                this.showToast('Required Field Missing', 'Please select whether Required Credentials are needed.', 'warning');
            } else if (this.credentialsNeeded === true && (!this.credentials || this.credentials.length === 0)) {
                this.showToast('Required Action', 'Please add at least one Required Credential before proceeding.', 'warning');
            }
    ...
    async handleCreateCredentialType() {
        ...
        try {
            ...
            this.newCredential = { ...this.newCredential, External_Contact_Credential_Type__c: created.Id };
        } catch (err) {
    ...
    handleCredentialsNeededChange(event) {
        ...
        this.credentialsNeeded = value === 'yes' ? true : (value === 'no' ? false : null);
