...
import getTrainingRequirements from '@salesforce/apex/OnboardingAppVendorProgramReqCtlr.getTrainingRequirements';
import createTrainingRequirement from '@salesforce/apex/OnboardingAppVendorProgramReqCtlr.createTrainingRequirement';
import getTrainingSystems from '@salesforce/apex/OnboardingAppVendorProgramReqCtlr.getTrainingSystems';
import createTrainingSystem from '@salesforce/apex/OnboardingAppVendorProgramReqCtlr.createTrainingSystem';
import updateTrainingRequirementRequiredStatus from '@salesforce/apex/OnboardingAppVendorProgramReqCtlr.updateTrainingRequirementRequiredStatus';
...
export default class VendorProgramOnboardingTrainingRequirements extends OnboardingStepBase {
    ...
    columnsExternal = [
        ...
        { label: 'Training System', fieldName: 'Training_System_Name', type: 'text' },
    ...
    get requiredTrainingRequirements() {
        ...
        return this.trainingRequirements.filter(req => req.Is_Required__c === true);
    }
    ...
    get requiredExternalRequirements() {
        return this.externalRequirements.filter(req => req.Is_Required__c === true);
    }

    get requiredInternalRequirements() {
        return this.internalRequirements.filter(req => req.Is_Required__c === true);
    }
    ...
    get hasRequiredTrainingRequirements() {
        return this.requiredTrainingRequirements && this.requiredTrainingRequirements.length > 0;
    }
    ...
    async refreshTrainingRequirements(preserveSelection = false, preservedSelectedIds = []) {
    ...
    handleFieldChange(event) {
        ...
        if (name === 'Training_System__c') {
            ...
            this.selectedTrainingSystemType = selectedSystem ? selectedSystem.System_Type__c : null;
    ...
    async handleCreate() {
        ...
        if (!this.vendorProgramId) {
            this.showError('Vendor Program ID is missing. Please refresh the page and try again.');
            return;
        }
        
        if (!this.newTraining.Training_System__c) {
            this.showError('Please select a Training System. This field is required.');
            return;
        }
        ...
        const record = {
            ...
            Sequence__c: this.newTraining.Sequence__c ? parseInt(this.newTraining.Sequence__c, 10) : null,
        ...
        if (this.selectedTrainingSystemType !== 'External' && this.newTraining.Training_Code__c) {
            record.Training_Code__c = this.newTraining.Training_Code__c;
        }

        try {
            const result = await createTrainingRequirement({ newRequirement: record });
            this.newTraining = {};
            this.showForm = false;
            ...
            this.showSuccess('Training requirement created successfully and added to the list below.');
        } catch (err) {
    ...
    handleFooterNextClick() {
        ...
        if (this.canProceed) {
            ...
            if (this.trainingNeeded === null) {
                this.showToast('Required Field Missing', 'Please select whether Training Requirements are needed.', 'warning');
            } else if (this.trainingNeeded === true && (!this.trainingRequirements || this.trainingRequirements.length === 0)) {
                this.showToast('Required Action', 'Please add at least one Training Requirement before proceeding.', 'warning');
            }
    ...
    async handleTrainingRequirementSelection(event) {
        ...
        this.selectedTrainingRequirementIds = newSelectedIds ? [...newSelectedIds] : [];
        
        // Find IDs that were added (moved to Selected for Group) - mark as Required
        const addedIds = newSelectedIds.filter(id => !previousSelectedIds.includes(id));
        
        // Find IDs that were removed (moved to Available) - mark as Not Required
        const removedIds = previousSelectedIds.filter(id => !newSelectedIds.includes(id));
    ...
    handleSelectionError(error, previousSelectedIds, addedIds, removedIds) {
        this.handleError(error, 'Failed to update Training Requirement required status.');
    ...
    handleTrainingNeededChange(event) {
        ...
        this.trainingNeeded = value === 'yes' ? true : (value === 'no' ? false : null);
    ...
    async handleCreateTrainingSystem() {
        ...
        try {
            ...
            if (this.newTrainingSystem.Active__c !== undefined && this.newTrainingSystem.Active__c !== null) {
            ...
            if (this.newTrainingSystem.System_Type__c) {
                trainingSystemToCreate.System_Type__c = this.newTrainingSystem.System_Type__c;
            }
            ...
            if (isActive) {
                ...
                this.showSuccess('Training System created successfully. Note: It is not marked as Active, so it will not appear in the dropdown. You can edit it later to activate it.');
            }
