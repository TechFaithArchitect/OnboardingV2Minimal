...
3. **Onboarding status** — **`OnboardingStatusNormalizationService`** loads **`Onboarding_Status_Normalization__mdt`**; **`OnboardingStatusEvaluatorService`** builds context from **`Onboarding__c`**, **`Onboarding_Requirement__c`**, **program template types**, and the **linked Opportunity** (stage only). **Contract / Agreement** fields are not read for predicates—keep **`Onboarding_Requirement__c`** in sync via automation. **Opportunity** participates in **Canceled** (and similar) paths: e.g. **`opportunityStageCanceled`** and the stage gate on **`anyRequirementCanceledWithStageGate`**, so a lost deal or reconciled requirement rows can drive status without conflicting with an open pipeline. First matching rule wins; DML updates **`Onboarding__c.Onboarding_Status__c`** and may update **Opportunity** stage from **`Target_Opportunity_Stage__c`**. See [ADR-001](adr/ADR-001-onboarding-status-evaluation-engine.md) and [onboarding-status-predicate-config.md](onboarding-status-predicate-config.md).
...
| **`OnboardingStatusEvaluatorService`** | Loads onboarding, linked **Opportunity** (stage), **`Onboarding_Requirement__c`** rows, and **active required `Vendor_Program_Requirement__c` types**; builds **`EvaluationContext`**; queries rules once per bulk call; uses injectable **`IOnboardingStatusRuleEngine`**; **`evaluateAndApply*`** DMLs onboarding + **one opportunity stage per opp** (first onboarding in bulk wins for stage). **Predicates** use requirement rows, CMDT normalization, onboarding header fields (e.g. **Assigned_Team__c**), and **Opportunity stage** for cancel/stage-gate rules — **not** linked Contract / Agreement record fields. Automation should keep **`Onboarding_Requirement__c.Status__c`** aligned with those linked records so status rules stay truthful. |
