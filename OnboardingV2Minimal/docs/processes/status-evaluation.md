...
**Note:** Flow implementations are being redesigned for V2. Any flow names referenced here are provisional and will be updated after the redesign.
...
- Triggered on `Onboarding__c` record changes
- Bypassed if:
  - User has `Onboarding_Bypass_Flow` permission
  - Status is "Setup Complete", "Expired", or "Denied"
...
- Set `Requirement__c` to `Vendor_Program_Requirement__c`
...
- Check rules are associated with the correct Vendor Program Group.
