...
- [x] 2026-03-19 Deployable hardening on Record Collection Editor write/read path completed: `RecordCollectionEditorConfigService` now executes child + relationship DML in `AccessLevel.USER_MODE`, enforces relationship-field create/update FLS gates before DML, and uses `WITH SECURITY_ENFORCED` for relationship existence lookup; `RecordCollectionEditorGetRecordsService` now safely reads optionally-unqueried fields (no runtime blow-up on sparse SOQL rows), applies `WITH SECURITY_ENFORCED` on fallback relationship query, and returns user-safe reference-bearing error messages with detailed internal logging. Validated by dry-run deploys `0AfRL00000dRpeH0AS` and `0AfRL00000dRpfu0AC`; deployed via `0AfRL00000dRlAs0AK` + `0AfRL00000dRqC90AK`; verification tests passed (`RecordCollectionEditorConfigServiceTest`, run `707RL00001Ih0Ym`, `4/4`).
