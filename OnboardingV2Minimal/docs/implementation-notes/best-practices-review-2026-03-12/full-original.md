...
Completed: audit of all classes containing `@AuraEnabled` or `@InvocableMethod` confirms exposed classes are declared `with sharing` (2026-03-18).
...
Completed: documented in "System-Mode Flow Exceptions" section below.
...
Completed: `OnboardingInvocables` and `OnboardingFollowUpInvocables` provide typed request/response DTO-based invocable facades for active onboarding operations.
...
Completed: `EmailCommTerritoryRoleHelperTest.testBulkUpdateMixedStateOver200Records` validates a 210-record mixed-state path and asserts operation-key idempotency behavior.
...
Completed: `TerritoryAssignmentsTrigger` routes by trigger context and operation, and `EmailCommTerritoryRoleHelper` enforces idempotency with operation-scoped processed keys.
- [x] Use `Trigger.newMap` / `oldMap` diffs to minimize unnecessary work.
Completed: `TerritoryAssignmentsTrigger` passes `Trigger.oldMap`, and helper logic refreshes only when tracked user-lookup fields change.
...
Completed: `DOMAIN_OmniSObject_SFL_CREATE_Fault_Message` centralizes fault-message normalization, trigger-reason generation, explicit queue-status overrides, queue-routing, and shared admin-context enrichment for communication and flow-error logging across `DOMAIN_OmniSObject_ACTION_Send_Communication_Template`, `BLL_Onboarding_SFL_Dispatch_Communication_By_Context`, `BLL_Training_Assignment_SCD_Training_Reminder_Emails`, the legacy `DOMAIN_OmniSObject_SFL_Send_Email_Communication` path, and newer onboarding / LearnUpon fault escalations.
...
Completed: Added `OnboardingRequirementSubjectInvocable` invocable that pre-filters subjects by existing `Unique_Key__c` before insert; `DOMAIN_OmniSObject_SFL_CREATE_Onboarding_Requirement_Subjects` now calls this invocable instead of recordCreates for idempotent retries.
...
Completed: repo audit confirms no `sfdx` command usage under `scripts/` or `Makefile`; automation targets use `sf` CLI.
- Note: when `UnsafeFilepathError` occurs on source-format deploy (cached temp path in source tracking), use `sf org disable tracking` or deploy via `--metadata` for targeted components; full metadata-format deploy (`sf project convert source` + `--metadata-dir`) bypasses source tracking but surfaces pre-existing org/repo drift.
...
Completed: Added `Makefile` with `make audit` and individual targets (`check-vpr-drift`, `build-legacy-queue`, `build-flow-deps`, `check-apex-refs`, `profile-flows`, `build-redundancy-audit`, `audit-fault-connectors`).
...
- Completed: `OnboardingInvocables.cls`, `OnboardingFollowUpInvocables.cls`, and `OnboardingInvocablesTest.cls` were added on 2026-03-12.
- [x] Add standard trigger-handler base/recursion guard where missing; add bulk safety tests.
- Completed: active trigger path `TerritoryAssignmentsTrigger` delegates to `EmailCommTerritoryRoleHelper` with operation-scoped idempotency, `oldMap`-diff filtering, stale-row cleanup, and a 210-record mixed-state test path (2026-03-12).
...
## System-Mode Flow Exceptions

The following flows run in `SystemModeWithoutSharing` by design. Rationale is documented for audit and future hardening.

| Flow | Rationale |
|------|-----------|
| `DOMAIN_OmniSObject_SFL_CREATE_Training_Assignment_Records` | Creates training assignments across accounts/contacts; may need cross-account visibility for LearnUpon sync. Called from `BLL_LearnUponContactEnrollment_RCD_Connect_Logical_Records` and training schedulers. |
| `DOMAIN_OmniSObject_SFL_CREATE_Required_External_Contact_Credenti` | Creates external contact credentials; may require system context for cross-object setup. |
| `DOMAIN_OmniSObject_SFL_GET_All_Related_Records` | Retrieval flow; system mode used for broad related-record visibility. |
| `DOMAIN_OmniSObject_SFL_CREATE_External_Contact_Credentials` | Creates credential records; may need system context for cross-org or admin scenarios. |

**Follow-up:** Evaluate whether each flow can be switched to default (user) mode; document any that must remain in system mode for compliance or cross-account requirements.
...
- Related shared rule/action paths: `BLL_BRE_Evaluate_Business_Rules`, `OnboardingStatusEvaluatorInvocable`, `OnboardingDefaultVendorProgramInvocable`, and associated CMDT records
...
- Completed: `Onboarding_Status_Normalization__mdt.agreement_paperwork_sent` is now present in canonical naming and active evaluator path.
- Completed: `make check-cmdt-alias-drift` now fails when onboarding CMDT alias duplicates reappear.
...
1. `P0` Flow fault wiring for the focal chain.
   - Completed 2026-03-18: Added fault connectors for all audited no-fault elements in `BLL_BRE_Evaluate_Business_Rules`, `BLL_Onboarding_Requirement_RCD_Logical_Process`, and `EXP_Opportunity_SCR_Create_Record`.
   - Completed 2026-03-18: Routed new focal fault paths through `DOMAIN_OmniSObject_SFL_CREATE_Fault_Message` with source flow/element context.
2. `P0` Output-contract hardening for focal flows.
   - Completed 2026-03-18: `DOMAIN_OmniSObject_SFL_CREATE_Related_Onboarding_Requirement_Records` now emits deterministic `errorMessage` output for the unsupported `OnboardingRecord` branch.
   - Completed 2026-03-18: `BLL_Onboarding_Requirement_RCD_Logical_Process` now inspects child/action `errorMessage` outputs and branches into shared fault handling.
   - Completed 2026-03-18: `EXP_Opportunity_SCR_Create_Record` now branches on async queue outcome (`queued` + message) and displays a failure screen path when queueing fails.
3. `P1` CMDT normalization and drift prevention.
   - Completed 2026-03-18 (targeted): added Agreement normalization for subject rollup status `Paperwork Sent` -> `Out for signature` so `ONLY_AGREEMENT_OUT_FOR_SIGNATURE` can evaluate correctly.
   - Completed 2026-03-18: canonicalized onboarding CMDT records to `Type__mdt.RecordName.md-meta.xml` and removed duplicate aliases (`157` removals, `4` migrations).
   - Completed 2026-03-18: added repo audit check `scripts/automation/check-cmdt-alias-drift.sh` and `make check-cmdt-alias-drift`.
   - Completed 2026-03-18: simplified evaluator by removing unused condition branches (`HAS_REQUIREMENTS_NOT_ALL_SETUP_COMPLETE`, `OPPORTUNITY_UNCANCELED`) to match active CMDT rules.
4. `P1` Apex and flow test hardening.
   - Completed 2026-03-18: expanded `OnboardingStatusEvaluatorTest` with deterministic evaluator-branch assertions via `@TestVisible` hook, null-input contract checks, and invocable apply/no-apply output-contract coverage.
   - Completed 2026-03-18: expanded default-vendor tests for scenario-missing, ambiguous match handling, and `Vendor_Program_Name_Contains__c` filtering branch.
   - Completed 2026-03-18: added static focal-flow validation script `scripts/automation/validate-focal-flow-error-paths.sh` and `make validate-focal-flow-error-paths` to enforce fault-path/error-propagation assertions across the targeted chain.
   - Completed 2026-03-18: added runtime Flow interview test `RelatedOnboardingReqFlowTest` to validate branch-level runtime error propagation/output contracts for `DOMAIN_OmniSObject_SFL_CREATE_Related_Onboarding_Requirement_Records`.
   - Completed 2026-03-18: prepared deploy manifest + post-destructive package and executed both dry-run (`0AfRL00000dPyqZ0AS`) and production (`0AfRL00000dQ7U10AK`) deployments with passing tests.
5. `P2` Audit tooling scope expansion.
   - Extend `scripts/automation/audit-flow-fault-connectors.sh` to optionally audit `subflows` and `recordLookups` so currently unscanned flow-element gaps are measurable.

### Evaluation (2026-03-18)
- Re-ran `scripts/automation/audit-flow-fault-connectors.sh` against `force-app/main/default/flows`; unresolved audit-scope gaps are now 42 (previously 51).
- Focal chain audit rows are all `yes`: `BLL_BRE_Evaluate_Business_Rules`, `BLL_Onboarding_Requirement_RCD_Logical_Process`, and `EXP_Opportunity_SCR_Create_Record`.
- Local XML validation passed for all four updated focal flows (`xmllint --noout`).
- Targeted dry-run deploy for agreement/subject-status alignment (`DOMAIN_OmniSObject_SFL_EVAL_Onb_Req_Subjects_By_Evidence`, `Onboarding_Status_Normalization.agreement_paperwork_sent`, `OnboardingStatusEvaluatorTest`) succeeded (`0AfRL00000dPrdl0AC`) prior to deployment.
- `make check-cmdt-alias-drift` now passes with zero onboarding CMDT alias duplicates after canonicalization.
- `make validate-focal-flow-error-paths` now passes `18/18` focal assertions (fault connectors + error propagation routes).
- Targeted dry-run deploy for evaluator/test hardening (`OnboardingStatusEvaluatorService`, `OnboardingStatusEvaluatorTest`, `OnboardingDefaultVendorProgramInvocTest`, `Onboarding_Default_Vendor_Program__mdt.REQUIRE_NDA_NAME_CONTAINS`) succeeded with `14/14` tests passing (`0AfRL00000dPzHx0AK`).
- Targeted dry-run deploy for runtime flow-interview + focal deployment manifest validation succeeded (`0AfRL00000dPyqZ0AS`) with `176` components and `18/18` tests passing.
- Production deployment for this focal wave succeeded in `OnboardV2` (`0AfRL00000dQ7U10AK`) with `176` components and `18/18` tests passing.
...
- Completed: explicit runtime error propagation now has static focal-flow assertions (`make validate-focal-flow-error-paths`) plus end-to-end Flow interview assertions in `RelatedOnboardingReqFlowTest`, validated in dry-run `0AfRL00000dPyqZ0AS` and production deploy `0AfRL00000dQ7U10AK`.
