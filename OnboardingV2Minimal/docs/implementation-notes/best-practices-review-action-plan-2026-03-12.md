...
- [ ] Build the remaining onboarding-requirement subject evaluator layer so real evidence sources update `Onboarding_Requirement_Subject__c.Status__c`; training assignments are now wired, with contract-backed compliance statuses, agreements, and credentials remaining next.
